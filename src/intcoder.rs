...
impl Intcode {
    ...
    pub fn new(to_copy: &Vec<i64>, log: bool) -> Intcode {
...
mod tests {
    ...
    fn test_immediate_eq() {
        ...
        let mut icoder = Intcode::new(&eq_8, true);
        let answer = icoder.run_single(8);
        assert_eq!(answer, 1);

        let mut icoder = Intcode::new(&eq_8, true);
        let answer = icoder.run_single(0);
        assert_eq!(answer, 0);
        
        let lt_8 = vec!(3,3,1107,-1,8,3,4,3,99);
        let mut icoder = Intcode::new(&lt_8, true);
        let answer = icoder.run_single(9);
        assert_eq!(answer, 0);
    ...
    fn test_pos_jmp() {
        ...
        let mut icoder = Intcode::new(&jmp, true);
        let answer = icoder.run_single(1);
        assert_eq!(answer, 0);
        
        let mut icoder = Intcode::new(&jmp, true);
        let answer = icoder.run_single(12);
        assert_eq!(answer, 0);
    ...
    fn test_long() {
        ...
        let mut icoder = Intcode::new(&long, true);
        let answer = icoder.run_single(7);
